...
class AuthWechat {
  ...
  //           alert(this.getAuthUrl(that.initConfig.appId))
		// })
        location.href = '/pages/users/login/index';
	}
    async toAuthNew() {
    		let that = this;
            let url = ''
    		await this.wechat().then(wx => {
    			url = this.getAuthUrl(that.initConfig.appId);
    		})
            return url
    	}
