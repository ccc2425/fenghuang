	 ...
	 class="store-home">
		...
		<view :style="{ height: `${(menuButtonInfo.top - systemInfo.statusBarHeight) * 2 + menuButtonInfo.height}px`, 'padding-right': `${(systemInfo.windowWidth - menuButtonInfo.left) * 2 + 34}rpx`}"
		 class="header">
			...
			<navigator :url="'/pages/store/list/index?mer_id='+id" hover-class="none" class="search"><text class="iconfont icon-xiazai5"></text>搜索商品/店铺名称</navigator>
		</view>
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<view class="header">
		...
		<scroll-view class="main" scroll-y="true" @scroll="scrollHome">
			...
			<view id="store" class="store">
				...
				<button hover-class="none" @click="followToggle">
			...
			set_where: function(param) {
				...
				switch (param) {
					...
					case 3:
						...
						this.order = this.sortPrice ? 'price_asc' : 'price_desc';
						break;
					case 4:
						this.navActive = 3;
						this.order = 'is_new';
