	...
	<view class="product-con">
		...
		<view>
			<scroll-view :scroll-top="scrollTop" scroll-y='true' scroll-with-animation="true" :style='"height:"+height+"px;"'
			 @scroll="scroll">
				<view id="past0">
					<image :src='storeInfo.image_input' :class='' style="width: 100%;"></image>
					<view class='wrapper'>
						...
						<view class='label acea-row row-between-wrapper' style="padding-bottom: 8px;">
							<view>总数量: {{storeInfo.num}}</view>
							<view>已出售: {{storeInfo.buy_num}}</view>
						</view>
					</view>
				...
				<view class='product-intro' id="past3">
					...
					<view class='conter' v-if="description" style="padding: 10px;">
		...
		<view class='footer acea-row row-between-wrapper'>
			...
			<view class='bnt acea-row'>
				<form @submit="op('confrim')" report-submit='true' v-if="attr.productSelect" style="width: 100%;">
					<button class='buy bnts' form-type="submit" :disabled="attr.productSelect.stock == 0" style="width: 100%;">立即夺宝</button>
					<view :hidden="userFeedbackHidden" class="popup_content">
					    <view class="popup_title">验证支付密码</view>
					    <view class="popup_textarea_item">
							<view style="margin-bottom: 10px;">点击确认按钮后将会从你的贡献值中扣除对应金额！</view>
		...
		<!-- <block v-if="sharePacket.max&&sharePacket.min">
			<shareRedPackets :sharePacket="sharePacket" @listenerActionSheet="listenerActionSheet" @closeChange="closeChange" :showAnimate="showAnimate" @boxStatus="boxStatus"></shareRedPackets>
		</block> -->
		<!-- 组件 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			op2: function() {
				let that = this;
			    lootorderPay(that.params).then(function(res) {
			        that.isShowbox = true
					// that.goloot()
			...
			getGoodsDetails: function() {
				...
				let that = this;
				getLootDetail(that.id).then(res => {
					uni.hideLoading();
			...
			getCouponList() {
				...
				getCoupons({
					...
					goodsArr = res.data
					getShopCoupons(that.storeInfo.mer_id).then(({data})=>{
