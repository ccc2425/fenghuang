					...
					<view class='item' :class='is_switch==true?"":"on"' hover-class='none' v-for="(item,index) in productList" :key="index"
					 @click="godDetail(item)">
						<view class='pictrue' :class='is_switch==true?"":"on"'>
							<image :src='item.image_input' :class='is_switch==true?"":"on"'></image>
						...
						<view class='text' :class='is_switch==true?"":"on"'>
							...
							<view class='money font-color' :class='is_switch==true?"":"on"'>
								数量：<text class='num'>{{item.num}} 
								</text>  份
							</view>
							<view class='money font-color'>
								夺宝金：<text>{{item.price}}</text>
							</view>
							<view class='money font-color'>
								已售：<text>{{item.buy_num}}</text>份
							</view>
		...
		methods: {
			...
			godDetail(item) {
				goShopDetail(item, this.uid).then(res => {
					uni.navigateTo({
						url: `/pages/loot_details/index?id=${item.id}`
					})
				})
			},
			// 组件确定
			...
			get_product_list: function(isPage) {
				...
				that.loading = true;
				that.loadTitle = '';
				getLootlist(that.where).then(res => {
